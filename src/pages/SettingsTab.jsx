...
export default function SettingsTab({ flat, onLeft }) {
  ...
  async function handleLeave() {
    if (!window.confirm('Leave this flat? You will need a new invite code to rejoin.')) return
  ...
  async function handleDeleteUtensil(utensilId) {
    if (!window.confirm('Delete this utensil?')) return
  ...
  async function handleEffortChange(utensilId, val) {
    const utensils = (flat.utensils || []).map(u =>
      u.id === utensilId ? { ...u, defaultEffort: Number(val) } : u
    )
    await updateUtensils(flat.id, utensils)
  }
        ...
        <div className="space-y-2">
          {flat.members?.map(m => (
            <div key={m.uid} className="flex items-center gap-3">
              {m.photoURL && <img src={m.photoURL} alt="" className="w-8 h-8 rounded-full" referrerPolicy="no-referrer" />}
          ...
          {flat.utensils?.map(u => (
            <div key={u.id} className="border border-gray-100 dark:border-gray-700 rounded-xl p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="range" min={1} max={100} value={u.defaultEffort}
                  onChange={e => handleEffortChange(u.id, e.target.value)}
                  className="flex-1 accent-blue-500"
                />
                <span className="text-xs font-semibold text-blue-600 w-6 text-right">{u.defaultEffort}</span>
              </div>
              {u.iconUrl && (
